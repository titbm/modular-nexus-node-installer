...
## Обновления интерфейса (последние изменения)

### Цветовая схема сообщений:
- **Задачи и результаты** (`core_task`, `core_result`) - белый цвет с галочками ✅
- **Статусы** (`core_status`) - желтый цвет для процессов выполнения
- **Ошибки** (`core_error`) - красный цвет с крестиками ❌
- **Заголовки блоков** - зеленый цвет с рамками из символов =
- **Инструкции** (`core_user_instruction`) - голубой цвет

### Структура блоков:
1. **ПРОВЕРКА СУЩЕСТВУЮЩИХ ПРОЦЕССОВ** - проверка и установка зависимостей (tmux, curl, jq)
...
- Галочки ✅ только белым цветом (без зеленой раскраски)
- Статусы процессов желтым цветом
- Таблицы с данными в рамках из символов ┌─┐│└┘
- Рамки заголовков из символов = (зеленый цвет)

### Примеры правильного оформления:
```bash
# Заголовок блока
core_block_header "НАЗВАНИЕ БЛОКА ЗАГЛАВНЫМИ"

# Задача (белая галочка)
core_task "Описание задачи"

# Статус (желтый)
core_status "Выполняем действие..."

# Результат (белая галочка)
core_result "Действие выполнено успешно"

# Ошибка (красный крестик)
core_error "Описание ошибки"
```

## Последние изменения в модуле system.sh

### Структура и порядок выполнения:
1. **Блок проверки зависимостей** (`system_check_dependencies`):
   - Проверка tmux, curl, jq
   - Автоматическая установка недостающих зависимостей
   - Определение пакетного менеджера (apt/yum/dnf)
   - Все сообщения с галочками ✅ белого цвета

2. **Блок поиска и завершения процессов** (`system_check_tmux_sessions`):
   - Проверка tmux сессий с nexus cli
   - Остановка активных сессий
   - Проверка и остановка процессов nexus через pgrep/pkill
   - Все сообщения с галочками ✅ белого цвета

### Принципы оформления в system.sh:
- Заголовки блоков заглавными буквами
- Все задачи и результаты белыми галочками ✅
- Статусы желтым цветом
- Ошибки красными крестиками ❌
- Логическое разделение на два основных блока

### Пример правильного использования функций system.sh:
```bash
# В главном скрипте:
...
### Требования к аналогичным модулям:
- Следовать той же цветовой схеме
- Использовать белые галочки для всех результатов
- Структурировать код логическими блоками
- Все заголовки блоков заглавными буквами
- Статусы процессов желтым цветом
- Обработка ошибок красными крестиками

## Архитектурные принципы проекта

### Структура модулей:
- **core.sh** - базовые функции вывода и утилиты
- **config.sh** - управление конфигурацией
- **system.sh** - проверка системы и зависимостей
- **memory.sh** - управление памятью и swap
- **swap.sh** - специализированные функции swap
- **nexus.sh** - установка и управление Nexus CLI
- **node.sh** - запуск и управление нодой
- **automation.sh** - настройка автоматизации (cron)

### Правила именования функций:
- Все функции модуля начинаются с префикса имени модуля
- Использовать snake_case для имён функций
- Примеры: `system_check_dependencies`, `memory_display_info`, `nexus_install`

### Правила загрузки модулей:
```bash
# Загрузка модуля с проверкой
if ! curl -s "https://raw.githubusercontent.com/titbm/modular-nexus-node-installer/main/modules/core.sh" | source /dev/stdin; then
    echo "❌ Ошибка загрузки модуля core.sh"
    exit 1
fi
```

### Обработка ошибок:
- Использовать `core_exit_error` для критических ошибок
- Возвращать коды ошибок в функциях
- Логировать все важные действия
- Проверять успешность выполнения команд

### Сохранение состояния:
- Использовать JSON для конфигурации
- Сохранять пользовательские настройки
- Восстанавливать состояние при перезапуске
- Файл конфигурации: `~/.nexus-installer-config.json`
