...
#### **curl (проверяется в основном скрипте):**
- Проверяется ПЕРЕД загрузкой модулей
- Необходим для загрузки всех модулей с GitHub
- Используется для API запросов к GitHub (проверка версий)
- Логично проверять до использования, а не после

#### **tmux (проверяется в system.sh):**
- Требуется для запуска Nexus CLI в фоновом режиме
- Позволяет пользователю закрыть терминал, не останавливая ноду
- Обеспечивает возможность просмотра логов

#### **jq (проверяется в system.sh):**
- Критически важен для работы с JSON конфигурацией
- Используется для парсинга ответов GitHub API
- Без jq невозможно сохранять/читать настройки пользователя
