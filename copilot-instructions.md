...
### Обоснование зависимостей system.sh:
...
#### **curl:**
- Нужен для загрузки модулей с GitHub при каждом запуске
- Используется для API запросов к GitHub (проверка версий)
- Хотя скрипт обычно запускается через curl, может использоваться и локально
